...
std::pair<std::list<Armor>, std::list<Lightbar>> Detector::detect_light(const cv::Mat & bgr_img, int frame_count)
{
  // 彩色图转灰度图
  cv::Mat gray_img;
  cv::cvtColor(bgr_img, gray_img, cv::COLOR_BGR2GRAY);
  ...
  armors.remove_if([&](const Armor & a) { return a.duplicated; });
  lightbars.remove_if([&](const Lightbar & a) {
    for (const auto & armor : armors) {
        if (a.id == armor.left.id || a.id == armor.right.id) {
            return true;  // 找到匹配，需要剔除
        }
    }
    return false;  // 没有匹配，保留
  });

  if (debug_) show_result(binary_img, bgr_img, lightbars, armors, frame_count);

  return std::make_pair(armors, lightbars);
}
